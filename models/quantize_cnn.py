...
class QuantizeEMAReset(nn.Module):
    def __init__(self, nb_code, code_dim, mu, gpu_id):
        super().__init__()
    ...
    def init_codebook(self, x):     # creating the codebook for the first time
        out = self._tile(x)
    ...
    def forward(self, x, temperature=0.0):
        N, width, T = x.shape
        ...
        code_idx = self.quantize(x, sample_codebook_temp=temperature)
...
class QuantizeEMA(nn.Module):
    ...
    def reset_codebook(self):
        ...
        self.code_sum = None
        self.code_count = None
        device = "cuda:" + str(self.gpu_id) if torch.cuda.is_available() else "cpu"
        self.register_buffer('codebook', torch.zeros(self.nb_code, self.code_dim).to(device))
