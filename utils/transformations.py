...
def d62rotmat(pose, device):
    pose = to_tensor(pose, device=device)
    reshaped_input = pose.reshape(-1, 6)
    return rotation_6d_to_matrix(reshaped_input)

def rotmat2d6(pose, device):
    pose = to_tensor(pose, device=device)
    return matrix_to_rotation_6d(pose)

def aa2rotmat(pose, device):
    pose = to_tensor(pose, device=device)
...
def rotmat2aa(pose, device):
    pose = to_tensor(pose, device=device)
    quat = matrix_to_quaternion(pose)
    return quaternion_to_axis_angle(quat)
    # reshaped_input = pose.reshape(-1, 3, 3)
    # quat = t3d.matrix_to_quaternion(reshaped_input)

def d62aa(pose, device):
    pose = to_tensor(pose, device=device)
    return rotmat2aa(d62rotmat(pose, device=device), device=device)

def aa2d6(pose, device):
    pose = to_tensor(pose, device=device)
    return rotmat2d6(aa2rotmat(pose,device=device), device)
